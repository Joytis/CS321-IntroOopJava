...
public class Connection
{
   ...
   */
   ...
   {
      if (state == RECORDING)
         system.addMessage(new Message(currentRecording));
      resetConnection();
   }
   ...
   */
   private void connect(String key)
   {
      if (key.equals("#"))
      {
         if (system.InitializeSystemMailbox(accumulatedKeys))
         {
            state = RECORDING;
            phone.speak(system.getGreeting());
         }
         else
   ...
   */
   private void login(String key)
   {
      if (key.equals("#"))
      {
         if (system.checkPasscode(accumulatedKeys))
         {
            state = MAILBOX_MENU;
   ...
   */
   private void changePasscode(String key)
   {
      if (key.equals("#"))
      {
         system.setPasscode(accumulatedKeys);
         state = MAILBOX_MENU;
         phone.speak(MAILBOX_MENU_TEXT);
         accumulatedKeys = "";
      }
   ...
   */
   private void changeGreeting(String key)
   {
      if (key.equals("#"))
      {
         system.setGreeting(currentRecording);
         currentRecording = "";
         state = MAILBOX_MENU;
         phone.speak(MAILBOX_MENU_TEXT);
      }
   ...
   */
   private void messageMenu(String key)
   {
      ...
      {
         String output = "";
         Message m = system.getCurrentMessage();
      ...
      {
         system.saveCurrentMessage();
         phone.speak(MESSAGE_MENU_TEXT);
      }
      else if (key.equals("3"))
      {
         system.removeCurrentMessage();
         phone.speak(MESSAGE_MENU_TEXT);
      }
